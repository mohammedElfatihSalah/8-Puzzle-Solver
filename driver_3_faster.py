...
def breadth_first_search(problem):
    ...
    for i in range(0 , 400000):
        ...
        for action in (problem.actions(node.state)):
            ...
            if explored_set.get(child_node.state_string, 0)==0 and frontier_set.get(child_node.state_string, 0) == 0:
...
def A_star_search(problem):
    ...
    while(len(frontier) > 0):
        ...
        for action in problem.actions(node.state):
            child_node = node.child_node(problem , action)
            if frontier_set.get(child_node.state_string,0) == 0 and explored_set.get(child_node.state_string,0) == 0:
                frontier_set[child_node.state_string] =1
